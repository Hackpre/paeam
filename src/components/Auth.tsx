...
export default function Auth() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (savedUser && email) {
      localStorage.setItem('paeam_token', 'demo-token-' + Date.now());
  ...
  const handlePaymentComplete = () => {
    const userData = { fullName, stageName, email, phone, nationalId, ipiNumber };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80">
          <div className="bg-neutral-900 rounded-2xl p-6 max-w-md w-full text-center">
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={handlePaymentComplete} className="flex-1 py-3 bg-gold-600 text-black font-semibold rounded-xl">Pay Now</button>
              <button onClick={handlePaymentComplete} className="flex-1 py-3 bg-neutral-800 text-white font-semibold rounded-xl">Pay Later</button>
            </div>
